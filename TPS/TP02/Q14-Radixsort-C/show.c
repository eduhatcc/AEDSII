...
int getMax(Show *s, int n) {
	int maior = s[0].releaseYear,
	    i = 1;

	while (i < n) {
		if (s[i].releaseYear > maior) {
			maior = s[i].releaseYear;
		}
		i++;
	}
...
void radCountingSort(Show *s, int n, int exp) {
	int count[10];
	Show ordenado[n];
	...
	// Ordenando
	for (int i = n-1; i >= 0; i--) {
		ordenado[count[(s[i].releaseYear / exp) % 10] -1] = s[i];
		count[(s[i].releaseYear / exp) % 10]--;
	}

	// Para itens do mesmo ano, faz ordenação por título (insertion sort)
	for (int i=0; i < n; i++) {
		Show tmp = ordenado[i];
		int j = i-1;
		bool fim = false;

		while (j >= 0 && (ordenado[j].releaseYear == tmp.releaseYear) && !fim) {
			char *lowA = strdup_lower(tmp.title);
			char *lowB = strdup_lower(ordenado[j].title);

			if (strcmp(lowA, lowB) < 0) {
				comparacoes += 2;
...
int main() {
    ...
    if (fgets(input, sizeof(input), stdin) != NULL) {
        ...
        clock_t start = clock();
        radixsort(shows, count);
        clock_t end = clock();
        double time = (double)(end - start) * 1000.0 / CLOCKS_PER_SEC;
        
        // Print log
        FILE *log = fopen("874201_radixsort.txt", "w");
        if (log) {
