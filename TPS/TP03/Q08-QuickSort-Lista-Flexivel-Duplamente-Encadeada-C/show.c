...
struct Celula {
    Show *elemento;
    Celula *prox;
    Celula *ant;  // For doubly linked list
};
...
Celula *new_celula() {
    ...
    tmp->elemento = NULL;
    tmp->prox = NULL;
    tmp->ant = NULL;  // For doubly linked list
    return tmp;
}
...
void quicksortLista(Celula *esq, Celula *dir) {
    if (esq != NULL && dir != NULL && esq != dir && esq->ant != dir->prox && esq != dir->prox) {
        ...
        while (j != dir) {
            comparacoes++;
            if (validacao(*j->elemento, *pivo) < 0) {
                i = (i == NULL) ? esq : i->prox;
        ...
        if (i != NULL && i->ant != NULL) {
            comparacoes++;
            quicksortLista(esq, i->ant);
        }
        if (i != NULL && i->prox != NULL) {
            comparacoes++;
            quicksortLista(i->prox, dir);
        }
    }
}

void quicksortBase(Lista *l) {
        quicksortLista(l->primeiro->prox, l->ultimo);
}
...
int main() {
    ...
    // Print file log
    FILE *log = fopen("874201_quicksort2.txt", "w");
    if (log) {
