...
void filaCirc_init(FilaCircular *filaCirc) {
	filaCirc->n = 0;
}

int filaCirc_media(FilaCircular *filaCirc) {
	int total = 0;
	int i = 0,
	    j = filaCirc->inicio;
	...
	while (i < filaCirc->n) {
		j = j % 5;
		total += filaCirc->array[j++]->releaseYear;
		i++;
	}

	int media = total/filaCirc->n;
	return media;
}

Show *filaCirc_remover(FilaCircular *filaCirc) {
	if (filaCirc->n == 0) return NULL;

	Show *tmp = filaCirc->array[filaCirc->inicio];
	filaCirc->inicio = (filaCirc->inicio +1) % 5;
	filaCirc->n--;
...
void filaCirc_inserir(FilaCircular *filaCirc, Show *s) {
	if ((filaCirc->n % 5) == 0) {
		filaCirc_remover(filaCirc);
	}

	filaCirc->array[filaCirc->fim] = s;
	filaCirc->fim = (filaCirc->fim +1) % 5;
	filaCirc->n++;
	printf("[Media] %d\n", filaCirc_media(filaCirc));
}

void filaCirc_mostrar(FilaCircular *filaCirc) {
	for (int i = filaCirc->n % 5; i > 0; i--) {
		printf("[%d] ", i-1);
		print_show(filaCirc->array[i]);
	}
}
...
int main() {
    ...
    FilaCircular filaCirc;
    filaCirc_init(&filaCirc);
    char buffer[256];
    bool fim = false;
    while (!fim)  {
        ...
        if (is_end(buffer)) {
            ...
            if (idx >= 0 && idx < data_count) {
                filaCirc_inserir(&filaCirc, &shows[idx]);
            }
        }
    }
    ...
    for (int i = 0; i < operacoes; i++) {
        ...
        if (op == 'I') {
            ...
            Show *rem = filaCirc_remover(&filaCirc);
            if (rem) {
                printf("(R) %s\n", rem->title);
            }
        }
