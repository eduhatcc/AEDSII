...
// Create new queue
FilaCircular *new_fila() {
    FilaCircular *tmp = malloc(sizeof(FilaCircular));
...
int fc_tamanho(FilaCircular *fc) {
   // fila vazia?
    if (fc->primeiro->prox == NULL) 
        return 0;

    int n = 1;
    Celula *i = fc->primeiro->prox->prox;  // já contamos o primeiro
    // enquanto não voltamos ao início, continuamos
    while (i != fc->primeiro->prox) {
        n++;
        i = i->prox;
    }
    return n;
...
int fc_media(FilaCircular *fc){
    int total = 0,
        n = fc_tamanho(fc),
        i = n;

    Celula *j = fc->primeiro->prox;

    while (i > 0) {
        total += j->elemento->releaseYear;
        j = j->prox;
        i--;
    }
...
// Remove 
Show *fc_remover(FilaCircular *fc) {
    if (fc->primeiro->prox == NULL) return NULL; // Empty queue
  
    Celula *tmp = fc->primeiro->prox;
    Show *ret = tmp->elemento;

     // Se for o único elemento
    if (tmp == fc->ultimo) {
        fc->primeiro->prox = NULL;
        fc->ultimo         = fc->primeiro;
    } else {
        // Avança o primeiro e refaz o laço
        fc->primeiro->prox = tmp->prox;
        fc->ultimo->prox   = tmp->prox;
    }
...
// Insert 
void fc_inserir(FilaCircular *fc, Show *s) {
    int n = fc_tamanho(fc);
    if (n == 5) {
        fc_remover(fc);
    }
    ...
    fc->ultimo->prox = tmp;
    fc->ultimo = tmp;
    fc->ultimo->prox = fc->primeiro->prox;
    printf("[Media] %d\n", fc_media(fc));
    tmp = NULL;
}

// Print list
void fc_mostrar(FilaCircular *fc) {
    int n = fc_tamanho(fc);
    Celula *i = fc->primeiro->prox;
    for (int j = 0; j < n; j++) {
        printf("[%d] ", j);
